...
<8> From now on, Tomcat will load availabl classes (both, system and application ones) from the shared archive if available.

If you're inerested in the current limitations of CDS/AppCDS, their implementation details and performance and memory consumption gains achievable by CDS/AppCDS please read on or visit my talk about https://fosdem.org/2018/schedule/event/class_data_sharing[Class Data Sharing at FOSDEM 2018].
