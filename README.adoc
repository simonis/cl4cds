...
For the Java VM, all the read-only parts of the loaded shared libraries (i.e. `libjvm.so`) can be shared between all the VM instances running at the same time. This explains why taking together, the two VM's consume less memory (i.e. have a smaller memory footprint) than the simple sum of their single resident set sizes. Notice that already the first instance had a PSS value which was smaller than the process' RSS value, because it used parts of libraries (i.e. `libc.so`) which were already mapped into the memory by other processes.
